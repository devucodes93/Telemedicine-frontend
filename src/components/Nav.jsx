...
const Nav = () => {
  ...
  const getNavLinkClass = ({ isActive }) => {
    ...
    return isActive
      ? `${baseClasses} text-lime-200 underline scale-105 pb-1`
      : `${baseClasses} hover:scale-105 hover:text-lime-100`;
  };
  ...
  const menuItems = [
    ...
    { to: "/patient-option", label: "Emergency", className: "bg-red-600 text-white px-3 py-1 rounded-md" },
  ];
  ...
  return (
    <nav className="bg-gradient-to-r from-emerald-500 to-green-600 text-white shadow-md relative top-0 z-50">
          ...
          {user && user.role !== "Doctor" && (
            <li>
              <select
                ...
                className="bg-white text-emerald-600 px-2 py-1 rounded shadow focus:outline-none font-semibold"
                style={{ minWidth: 120 }}
              >
      ...
      {/* Mobile Sidebar */}
      <div
        className={`fixed top-0 left-0 w-64 sm:w-72 h-full bg-gradient-to-b from-emerald-500 to-green-700 shadow-xl transform transition-transform duration-300 z-50 ${
      ...
      >
        ...
        {/* Sidebar Menu */}
        <ul className="flex flex-col items-start gap-6 p-6 text-lg font-medium text-white">
          {menuItems.map((item) => (
            <li key={item.to}>
              <NavLink
                to={item.to}
          ...
          {user && user.role !== "Doctor" && (
            <li className="w-full mt-4">
              ...
              <select
                ...
                value={language}
                onChange={handleLanguageChange}
                className="bg-white text-emerald-600 px-2 py-1 rounded shadow focus:outline-none font-semibold w-full"
                style={{ minWidth: 120 }}
              >
                {languageOptions.map((opt) => (
                  <option key={opt.code} value={opt.code}>
